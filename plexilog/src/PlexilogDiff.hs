...
import PLEXIL.LogParser (parseMaudeTrace,seqTrace,seqTrace,Event,SeqTrace)
...
main = do
    ...
    plexilTrace <- case filterAndParseTrace plexilLog of
...
test :: IO ()
test = do Right plexilTrace <- filterAndParseTrace      <$> readFile "output.0.log"
...
testPlexil =
    do Right plexilTrace <- filterAndParseTrace      <$> readFile "output.0.log"
       let seqPlexilTrace = seqTrace plexilTrace
       putStrLn "Plexil trace: "
       putStrLn $ show seqPlexilTrace
