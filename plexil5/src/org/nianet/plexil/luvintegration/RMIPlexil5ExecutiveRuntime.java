...
import org.nianet.plexil.luvintegration.remotecommands.RMICommandMonitor;
import org.nianet.plexil.luvintegration.remotecommands.RMIRequestType;
import org.nianet.plexil.luvintegration.remotecommands.RemoteStepService;
import org.nianet.plexil.luvintegration.remotecommands.RemoteStepServiceImpl;
...
public class RMIPlexil5ExecutiveRuntime {

	private static final String _REMOTESTEPSERVICE = "REMOTESTEPSERVICE";
	private static final int _RMI_PORT = 32000;
	...
	private static void setupRMIServices() throws RemoteException, AlreadyBoundException, UnknownHostException{
		
		//String host=InetAddress.getLocalHost().getHostAddress();
		String host="localhost";
		
		RemoteStepServiceImpl rss=new RemoteStepServiceImpl();
		RemoteStepService stub=(RemoteStepService)UnicastRemoteObject.exportObject(rss,0);

		LocateRegistry.createRegistry(_RMI_PORT);

		Registry registry=LocateRegistry.getRegistry(host,_RMI_PORT);
		registry.bind(_REMOTESTEPSERVICE, stub);

	}
	

	public static void main(String[] args) throws IOException, PlexilPlanLoadException, MaudeRuntimeConfigurationException, MaudeOutputSynchronizationException, ParseException, InterruptedException, MaudeRuntimeException, AlreadyBoundException, PlexilScriptException {

		System.out.println("RMI-PLEXIL5 Executive.");
		setupRMIServices();
		...
		String xmlPlexilPlan=readXMLData(plxPlanFile.getAbsolutePath());
		...
		PlexilPlan pp=PlexilXMLUnmarshaller.getInstance().getPlexilPlanFromPLX(new File(xmlPlexilPlan));		

		out.write(standardOutput[0]);
		out.write(_STANDARD_UE_MESSAGE_END);

		out.write(xmlPlexilPlan);
		out.write(_STANDARD_UE_MESSAGE_END);
		...
		do{
			synchronized (RMICommandMonitor.monitor) {
				RMICommandMonitor.monitor.wait();
			}			
			
			if (RMICommandMonitor.monitor.getLastRequest().equals(RMIRequestType.MICRO)){
				ns=eng.microStep();	
			}
			else{
