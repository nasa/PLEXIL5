    ...
    for maude_plan in maude_plans:
        if str(args.test) in str(maude_plan) or args.test is None:
            ...
            with open('semantics/test/regression/' + 'run.maude', 'w') as file:
                file.write(f'''load ../../src/plexil-v.maude
...
'''
            ...
            run_command_with_timeout('plexiltest -p '+ plans_root + maude_plan[:-6] + '.plx' + ' -L ' + plans_root + ' -s ' + scripts_root + plan_script + ' -d semantics/test/acceptance/Debug.AcceptanceTest.cfg' + ' > semantics/test/regression/output.plexil 2>&1',5)
            print('\n\n' + '--------------------------------' + maude_plan + '--------------------------------' + '\n')
